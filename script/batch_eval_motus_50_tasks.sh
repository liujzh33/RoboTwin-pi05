...
mkdir -p "$LOG_DIR"
MAIN_LOG="$LOG_DIR/batch_eval_$(date +%Y%m%d_%H%M%S).log"
...
echo "==========================================" | tee -a "$MAIN_LOG"
echo "批量评估开始时间: $(date)" | tee -a "$MAIN_LOG"
...
for i in "${!TASKS[@]}"; do
    ...
    echo "" | tee -a "$MAIN_LOG"
    echo "==========================================" | tee -a "$MAIN_LOG"
    echo "[$TASK_NUM/$TOTAL_TASKS] 开始任务: $TASK_NAME" | tee -a "$MAIN_LOG"
    echo "开始时间: $(date)" | tee -a "$MAIN_LOG"
    echo "==========================================" | tee -a "$MAIN_LOG"
    
    # 为每个任务创建单独的日志文件
    TASK_LOG="$LOG_DIR/${TASK_NAME}_$(date +%Y%m%d_%H%M%S).log"
    
    # 运行任务（直接运行并捕获错误）
    # 注意：如果任务出错，会停止当前任务并继续下一个
    if CUDA_VISIBLE_DEVICES=1 python "$SCRIPT_PATH" \
        --config "$CONFIG_PATH" \
        --overrides \
        --task_name "$TASK_NAME" \
        ...
        # 任务成功完成
        echo "[$TASK_NUM/$TOTAL_TASKS] ✅ 任务完成: $TASK_NAME" | tee -a "$MAIN_LOG"
        ...
        # 任务失败
        EXIT_CODE=$?
        echo "[$TASK_NUM/$TOTAL_TASKS] ❌ 任务失败: $TASK_NAME (退出码: $EXIT_CODE)" | tee -a "$MAIN_LOG"
...
echo "" | tee -a "$MAIN_LOG"
echo "==========================================" | tee -a "$MAIN_LOG"
echo "批量评估完成时间: $(date)" | tee -a "$MAIN_LOG"
echo "==========================================" | tee -a "$MAIN_LOG"
echo "总任务数: $TOTAL_TASKS" | tee -a "$MAIN_LOG"
