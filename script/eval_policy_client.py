...
class ModelClient:
    ...
    def set_language(self, instruction):
        """Set language instruction for the model"""
        return self.call('set_language', obs=instruction)
    
    def update_observation_window(self, input_rgb_arr, input_state):
        """Update observation window with new RGB arrays and state"""
        return self.call('update_observation_window', obs={'rgb': input_rgb_arr, 'state': input_state})
    
    def get_action(self):
        """Get action from the model"""
        return self.call('get_action')
    
    def reset_obsrvationwindows(self):
        """Reset observation windows"""
        return self.call('reset_obsrvationwindows')
    
    # 添加属性以兼容eval函数中的检查
    @property
    def observation_window(self):
        # 对于远程模型，我们无法直接访问observation_window
        # 返回None表示需要设置language
        return None
    
    @property
    def pi0_step(self):
        # 返回默认的action horizon，实际值应该在服务器端
        return 50
...
def main(usr_args):
    ...
    port = usr_args["port"]
    host = usr_args.get("host", "localhost")  # 支持从配置中读取host，默认为localhost
    save_dir = None
    video_save_dir = None
    video_size = None
    ...
    # 如果使用ModelClient（指定了port），则不需要本地加载模型，但仍需要policy_name用于eval函数
    use_remote_model = port is not None
    if not use_remote_model:
        if policy_name is None:
            raise ValueError("policy_name must be specified when not using remote model server")
        get_model = eval_function_decorator(policy_name, "get_model", conda_env=policy_conda_env)
    else:
        # 使用远程模型时，仍然需要policy_name用于eval函数和保存路径
        if policy_name is None:
            # 尝试从配置文件中读取，如果没有则使用默认值
            policy_name = usr_args.get("policy_name", "pi05")  # 默认使用pi05
    ...
    # 检查是否使用WebSocket服务器
    # 如果服务器是websocket_policy_server（通过serve_policy.py运行），使用WebsocketModelClient
    use_websocket = usr_args.get("use_websocket", False)
    if use_websocket:
        from websocket_model_client import WebsocketModelClient
        model = WebsocketModelClient(host=host, port=port)
    else:
        model = ModelClient(host=host, port=port)  # TCP Socket客户端（用于policy_model_server.py）
    
    open_gripper = usr_args.get("open_gripper", False)
...
def eval_policy(task_name,
    ...
    # 即使使用远程模型，也需要本地的eval函数来协调调用
    if policy_name is None:
        raise ValueError("policy_name must be specified (needed for eval function)")
    ...
    while succ_seed < test_num:
        ...
        succ = False
        # 调用模型的重置方法（注意：方法名是 reset_obsrvationwindows，不是 reset_model）
        model.call(func_name='reset_obsrvationwindows')
        # Reset gripper intervention flags for new episode
        reset_func = eval_function_decorator(policy_name, "reset_model", conda_env=policy_conda_env)
        reset_func(model)
        
        # Get arm dimensions for gripper intervention
        ...
        while TASK_ENV.take_action_cnt < TASK_ENV.step_lim:
            ...
            # Pass open_gripper flag and arm dimensions to eval function
            if open_gripper:
...
def parse_args_and_config():
    ...
    parser.add_argument("--host", type=str, help="Host address for model server (default: localhost)")
    parser.add_argument("--use-websocket", action="store_true", help="Use WebSocket client (for websocket_policy_server)")
    parser.add_argument("--open-gripper", action="store_true", help="Force open gripper when closed to test recovery ability")
