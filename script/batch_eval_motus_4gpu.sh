...
mkdir -p "$LOG_DIR"
MAIN_LOG="$LOG_DIR/batch_eval_4gpu_$(date +%Y%m%d_%H%M%S).log"
...
for i in "${!ALL_TASKS[@]}"; do
    ...
    if ! check_task_completed "$task_name"; then
        ...
        echo "[跳过] 任务已完成: $task_name" | tee -a "$MAIN_LOG"
    fi
done
...
echo "" | tee -a "$MAIN_LOG"
echo "==========================================" | tee -a "$MAIN_LOG"
echo "剩余未完成任务数: $TOTAL_REMAINING / 50" | tee -a "$MAIN_LOG"
echo "==========================================" | tee -a "$MAIN_LOG"
...
for idx in "${!TASKS[@]}"; do
    ...
    echo "  [$original_idx] ${TASKS[$idx]}" | tee -a "$MAIN_LOG"
done
echo "==========================================" | tee -a "$MAIN_LOG"
...
for i in "${!TASKS[@]}"; do
    ...
    echo "" | tee -a "$MAIN_LOG"
    echo "==========================================" | tee -a "$MAIN_LOG"
    echo "[GPU $GPU_ID] [$TASK_NUM/$TOTAL_TASKS] 开始任务: $TASK_NAME" | tee -a "$MAIN_LOG"
