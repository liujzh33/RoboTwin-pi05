...
# This script tests model's self-recovery ability by forcing open gripper when closed (30 frames)
...
MAIN_LOG="$LOG_DIR/batch_eval_5gpu_open_gripper_${TIMESTAMP}.log"
...
if [ ! -d "$TORCH_CHECKPOINT_DIR" ]; then
    echo "❌ Error: Checkpoint directory not found: $TORCH_CHECKPOINT_DIR"
    exit 1
fi

echo "✅ Checkpoint directory verified: $TORCH_CHECKPOINT_DIR"
echo ""
...
for i in "${!GPUS[@]}"; do
    gpu_id=${GPUS[$i]}
    echo "启动 GPU ${gpu_id} 的评估任务 (带夹爪恢复测试)..."
...
for i in "${!GPUS[@]}"; do
    gpu_id=${GPUS[$i]}
    echo "  GPU ${gpu_id}: tail -f ${LOG_DIR}/gpu${gpu_id}_open_gripper_${TIMESTAMP}.log"
done
...
# Save PID info to main log
{
    ...
    echo ""
    echo "Failed tasks will be saved to:"
    for i in "${!GPUS[@]}"; do
        gpu_id=${GPUS[$i]}
