...
class PaligemmaTokenizer:
    def __init__(self, max_len: int = 48):
        ...
        logging.info(f"[PaligemmaTokenizer] Initialized with max_token_len={max_len}")

        path = download.maybe_download("gs://big_vision/paligemma_tokenizer.model", gs={"token": "anon"})
        with path.open("rb") as f:
            self._tokenizer = sentencepiece.SentencePieceProcessor(model_proto=f.read())

    def tokenize(self, prompt: str, state: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Tokenize a prompt (and optionally state) for PI0/PI05 models.
        
        For PI05 inference with subtask generation:
        - IGNORE state parameter (state should NOT be in the prompt during inference)
        - Use format: "Task: {prompt}. Subtask: " (model will generate subtask)
        
        Returns:
            tokens: Tokenized sequence
            mask: Token mask (True for valid tokens)
            loss_mask: Loss mask (True where loss should be computed, i.e., on all prompt tokens)
        """
        import string
        
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        
        # CRITICAL FIX: For PI05 inference, IGNORE state parameter
        # The old format "Task: {prompt}, State: {state}; Action:" is WRONG for subtask generation
        # Correct format: "Task: {prompt}. Subtask: " (let model generate subtask)
        if state is not None:
            # This path is for PI05 inference - format for subtask generation
            cleaned_text = cleaned_text.lower()
            # Remove the last punctuation character if present and add period
            if cleaned_text and cleaned_text[-1] in string.punctuation:
                cleaned_text = cleaned_text[:-1]
            cleaned_text += '.'
            full_prompt = f"Task: {cleaned_text} Subtask: "
            tokens = self._tokenizer.encode(full_prompt, add_bos=True)
            # For PI05, compute loss on all tokens (for subtask generation)
            loss_mask = [True] * len(tokens)
        else:
            # This is the Pi0 format, where the state is part of the continuous action expert input.
            # tokenize "\n" separately as the "start of answer" token
            tokens = self._tokenizer.encode(cleaned_text, add_bos=True) + self._tokenizer.encode("\n")
            # For PI0, compute loss on all tokens
            loss_mask = [True] * len(tokens)
        ...
        return np.asarray(tokens), np.asarray(mask), np.asarray(loss_mask)
    ...
    def tokenize_high_low_prompt(self, high_prompt: str, low_prompt: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        Format: "Task: {high_prompt}. Subtask: {low_prompt};\nAction: "
        ...
        Returns:
            tokens: Tokenized sequence
            mask: Token mask (True for valid tokens)
            ar_mask: Autoregressive mask (True for autoregressive tokens)
            loss_mask: Loss mask (True where loss should be computed, i.e., on subtask tokens)
        """
        import string
        ...
        if tokens_len < self._max_len:
            ...
            # Token padding should be 0 (integer), not False (boolean)
            token_padding = [0] * padding_len
            # Mask padding should be False (boolean)
            mask_padding = [False] * padding_len
            
            mask = [True] * tokens_len + mask_padding
            tokens = tokens + token_padding
            ar_mask = ar_mask + mask_padding
            loss_mask = loss_mask + mask_padding
        else:
            if len(tokens) > self._max_len:
                logging.warning(
                    f"Token length ({len(tokens)}) exceeds max length ({self._max_len}), truncating. "
