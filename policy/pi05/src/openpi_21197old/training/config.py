...
_CONFIGS = [
    ...
    # pi05_base by full (all tasks)
    TrainConfig(
        name="pi05_aloha_full_base_all",
        model=pi0_config.Pi0Config(pi05=True),
        data=LeRobotAlohaDataConfig(
            repo_id="robotwin_pi05_all",
            repack_transforms=_transforms.Group(inputs=[
                _transforms.RepackTransform({
                    "images": {
                        "cam_high": "observation.images.cam_high",
        ...
        weight_loader=weight_loaders.CheckpointWeightLoader("/mnt/data/guoxiaoyu/cache/modelscope/models/BITkkgzy/pi05_base/pi05_base/params"),
        num_train_steps=30000,
        num_workers=32,
        batch_size=192,
        fsdp_devices=6,  # 6x80G A100
    ),
    # pi0_base by lora
    TrainConfig(
