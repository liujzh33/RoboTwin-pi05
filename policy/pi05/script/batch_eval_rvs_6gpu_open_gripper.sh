...
# 并行批量运行20个任务（open_gripper 扰动评估，6个GPU版本）
...
RVS_SCRIPT="$ROOT/script/run_eval_rvs_open_gripper.sh"
...
mkdir -p "$LOG_DIR"
# 清除旧的日志文件（只清除当前GPU的 open_gripper 任务日志，不删其它脚本的日志）
rm -f "$LOG_DIR"/gpu${GPU_ID}_*_open_gripper_*.log 2>/dev/null
MAIN_LOG="$LOG_DIR/batch_eval_rvs_6gpu_open_gripper_$(date +%Y%m%d_%H%M%S).log"
...
# 检查任务是否已完成（open_gripper 结果目录名含 open_gripper）
check_task_completed() {
    ...
    # 结果路径: eval_result/task_name/pi05_rvs/env_config/rvs_10000_reward_*_open_gripper/{timestamp}/_result.txt
    local result_dir="$RESULT_BASE_DIR/$task_name/pi05_rvs/$env_config"
    if [ -d "$result_dir" ]; then
        local result_file=$(find "$result_dir" -path "*open_gripper*/*/_result.txt" -type f 2>/dev/null | head -1)
...
echo "使用 RvS 权重 + open_gripper 扰动（eval_policy_initial_motus_open_gripper.py）" | tee -a "$MAIN_LOG"
echo "" | tee -a "$MAIN_LOG"
...
for i in "${!TASKS[@]}"; do
    ...
    # 为每个任务创建单独的日志文件
    TASK_LOG="$LOG_DIR/gpu${GPU_ID}_${TASK_NAME}_${ENV_CONFIG}_open_gripper_$(date +%Y%m%d_%H%M%S).log"
    
    # 运行任务（使用 run_eval_rvs_open_gripper.sh 脚本）
