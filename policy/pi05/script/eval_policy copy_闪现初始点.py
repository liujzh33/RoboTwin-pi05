    ...
    def _try_multi_env_recovery(
        ...
        for variant in range(1, num_stages + 1):
            ...
            try:
                ...
                # CRITICAL: For recovery, we need to skip robot initialization movements
                # setup_demo() calls _init_task_env_() which includes:
                #   - robot.move_to_homestate() - moves robot to initial position (WE WANT TO SKIP THIS)
                #   - together_open_gripper() - opens grippers (we'll restore from snapshot instead)
                #   - robot.set_origin_endpose() - sets origin pose (not needed for recovery)
                #   - load_actors() - loads objects (will reset positions, but we'll restore them)
                #
                # Solution: Call _init_task_env_() but immediately restore robot state
                # This way robot never visually moves to initial position
                
                # Call _init_task_env_() for basic setup (table, robot, camera, actors)
                # But we'll immediately restore robot state, so move_to_homestate() effect is overridden
                env._init_task_env_(
                ...
                # CRITICAL: Immediately restore robot state BEFORE any video saving
                # This ensures robot never visually appears at initial position
                # The move_to_homestate() was called, but we immediately override it
                print(f"\033[94m[Recovery] [Variant {variant}] Restoring robot and object states directly (overriding initialization)\033[0m")
                _restore_env(env, snapshot)
                ...
                if actor_poses:
                    ...
                    restore_all_actor_poses(env, actor_poses)
                    # Step simulation multiple times to ensure poses are applied
                    for _ in range(10):
                        env.scene.step()
                        if env.render_freq:
                            env._update_render()
