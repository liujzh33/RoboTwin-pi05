...
Usage (from policy/pi05):
    python scripts/process_data_data_new.py <task_dir_name> [--data-root ../../data/data_new]
Options:
    --data-root      Root folder containing the task directories (default: ../../data/data_new)
    --output-root    Where to place processed_data (default: processed_data)
    --desc-type      Which instruction set to use from metadata (default: seen)
    --max-episodes   Optional cap on number of episodes to process (default: auto from collection_stats.json or data/*.hdf5)
"""
...
def find_episode_count(task_path: str, max_episodes: int | None) -> int:
    """Infer episode count from collection_stats.json or data/*.hdf5."""
...
def load_instructions(metadata_path: str, desc_type: str) -> Tuple[List[str], bool]:
    with open(metadata_path, "r") as f:
        meta = json.load(f)
    instr_dict = meta.get("episode_instructions", {}) or {}

    # 1) Prefer requested desc_type if non-empty
    instructions = instr_dict.get(desc_type) or []
    if instructions:
        return instructions, False

    # 2) Fallback to any other non-empty list in episode_instructions
    for val in instr_dict.values():
        if val:
            return val, False

    # 3) Nothing found -> mark missing
    return [], True
...
def main():
    parser = argparse.ArgumentParser(description="Process data_new task into processed_data format.")
    parser.add_argument(
        "task_dir",
        type=str,
    ...
    save_path = os.path.join(args.output_root, f"{args.task_dir}-{episode_num}")
    ...
    for i in range(episode_num):
        ...
        instructions, missing = load_instructions(paths.metadata_path, args.desc_type)
        if missing:
    ...
    if missing_episodes:
        print(f"[summary] missing instructions for episodes: {missing_episodes}")
