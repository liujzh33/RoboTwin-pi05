...
class TokenizeHighLowPrompt(DataTransformFn):
    ...
    def __call__(self, data: DataDict) -> DataDict:
        if (high_prompt := data.pop("high_prompt", None)) is None:
            raise ValueError("high_prompt is required")
        if (low_prompt := data.pop("low_prompt", None)) is None:
            raise ValueError("low_prompt is required")
...
class LoadSubtaskFromInstructions(DataTransformFn):
    ...
    Expects data to have 'instructions' and 'subtasks' keys (from instructions.json).
    Randomly selects one instruction and its corresponding subtask.
    """
    use_first_subtask: bool = False  # If True, use first subtask; if False, randomly select

    def __call__(self, data: DataDict) -> DataDict:
        if "instructions" not in data:
            raise ValueError("instructions key is required in data")
        if "subtasks" not in data:
            raise ValueError("subtasks key is required in data (data should come from instructions.json)")
        ...
        # Randomly select one instruction and its corresponding subtask
        if self.use_first_subtask:
            idx = 0
        else:
            idx = np.random.randint(0, len(instructions))

        high_prompt = instructions[idx]
        # For now, use the first subtask from the subtask list
        # You can modify this to use a specific subtask or combine multiple subtasks
        if isinstance(subtasks[idx], list) and len(subtasks[idx]) > 0:
            low_prompt = subtasks[idx][0]  # Use first subtask
        else:
