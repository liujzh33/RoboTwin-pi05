...
## Pi0.5 评测时接入 Judge（`--use_judge_model`）

在 `policy/pi05` 下：

```bash
source .venv/bin/activate
export XDG_CACHE_HOME=/path/to/pi05/.cache   # 按需
# Pi0.5 用物理 GPU 7，Judge 用物理 GPU 1（会自动 export CUDA_VISIBLE_DEVICES=7,1）
bash eval.sh beat_block_hammer demo_randomized <train_config> <exp_name> 0 7 16000 --use_judge_model --judge_gpu=1
```

- 不加 `--judge_gpu` 时 Pi0.5 与 Judge **共用**同一张卡（`cuda:0`），显存压力大。
- 双卡时：**Pi0.5 → `cuda:0`（对应第一个物理 GPU），Judge → `cuda:1`（第二个物理 GPU）**。
- 启动后应看到绿色行：`[deploy_policy_with_judge] Module loaded successfully`；若仍是普通 `[Task]` 无该行，请删掉 `policy/pi05/__pycache__` 后重跑。
- **调试落盘**：`policy/pi05/judge_debug/episode_*_时间戳/`  
  - `start_*.png`：每局首帧三视角  
  - `instruction.txt`：本局指令  
  - `trigger_step{N}_*.png`：触发 Judge 当帧三视角  
- Judge 仅在触发条件满足时加载（显存占用出现在首次触发后）；两次调用间隔有 **50 step 冷却**，避免 stall 条件每步重复打 Judge。
- **启动即占满 Judge 卡**：`get_model` 返回前会调用 `_get_judge_client()`（默认开启）。若只想首次触发再加载，可设 `export JUDGE_PRELOAD_AT_START=0`。
- 环境变量（可选）：`JUDGE_BASE_MODEL`、`JUDGE_LORA_PATH`、`JUDGE_DEVICE`、`JUDGE_PRELOAD_AT_START`。
