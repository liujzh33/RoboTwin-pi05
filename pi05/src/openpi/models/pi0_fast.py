...
class Pi0FAST(_model.BaseModel):
    def __init__(self, config: Pi0FASTConfig, rngs: nnx.Rngs):
        ...
        if hasattr(config, 'n_obs_steps') and config.n_obs_steps > 1 and fake_image.ndim == 5:
    ...
    def embed_inputs(
        ...
        if token_ar_mask.ndim == 3:
            token_ar_mask = token_ar_mask[:, 0, :]
        
        tokenized_inputs_embeddings = self.PaliGemma.llm(tokenized_prompt, embed_only=True)
