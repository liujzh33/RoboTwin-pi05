...
class LoadSubtaskFromInstructions(DataTransformFn):
    ...
    # Recovery 段（同时含 [Correction] 与 [Subtask]）：
    # - False（默认）：correction 文本并入 high_prompt，仅 subtask 文本进 low_prompt（旧行为；CE 只压在 subtask 上）。
    # - True：high_prompt 仅保留任务 instruction，整段「correction + subtask」留在 low_prompt，
    #   经 TokenizeHighLowPrompt 后整段进入带 loss 的 AR 后缀，CE 同时监督 correction 与 subtask。
    recovery_joint_low_supervision: bool = False
    ...
    def __call__(self, data: DataDict) -> DataDict:
        ...
        #     default: high_prompt := "{instruction} {correction_text}", low_prompt := "{subtask_text}"
        #     recovery_joint_low_supervision=True: high_prompt := instruction only,
        #         low_prompt := full raw line (correction + subtask) for joint CE on the AR suffix.
        ...
        if isinstance(low_prompt_raw, str):
            ...
            if "[Correction]" in s and "[Subtask]" in s:
                if self.recovery_joint_low_supervision:
                    high_prompt = instructions[idx]
                    low_prompt = s
                else:
