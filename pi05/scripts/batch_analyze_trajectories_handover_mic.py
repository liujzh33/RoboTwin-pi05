...
ANALYZE_SCRIPT_NAME = "analyze_trajectory_handover_mic.py"
...
def process_all_episodes(data_dir: str, output_dir: str):
    ...
    for ep_dir in episodes:
        ...
        if not hdf5_file.exists():
            continue
        save_file = out_path / f"analysis_episode_{match.group(1)}.png"
        print(f"Processing Episode {match.group(1)}...", end=" ", flush=True)
        try:
            subprocess.run(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", required=True, help="processed_data/.../handover_mic-...-N")
