#!/usr/bin/env python3
"""
handover_block 轨迹分析：4 面板图（夹爪、末端高度 Z、速度、阶段划分）。
4 阶段：左靠近红块 → 抓住红块并移到中间 → 右抓红块、左放开 → 右放置到蓝块。
"""
...
from task_definitions.handover_block import HandoverBlockProcessor
from task_definitions.trajectory_analyzer import TrajectoryAnalyzer

RAW_DATA_ROOT = "/mnt/data1/liujingzhi/dataset/handover_block/aloha-agilex_randomized_500/data"
...
def analyze_episode(hdf5_path: Path, save_path: Path | None = None):
    ...
    task_processor = HandoverBlockProcessor()
    ...
    axes[3].set_ylim(0, 1)
    axes[3].set_yticks([])
    legend_note = "[L-Approach, Grasp+Move, R-Grasp+L-Release, R-Place]"
...
def main():
    parser = argparse.ArgumentParser(description="Handover Block trajectory analysis (4 phases)")
