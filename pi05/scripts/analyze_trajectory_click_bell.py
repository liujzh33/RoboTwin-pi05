#!/usr/bin/env python3
"""
click_bell 轨迹分析：4 子图（夹爪、双臂 Z 高度、速度、阶段），
3 阶段（夹爪 T1 + Z 轴 T2），前三个子图绘制 checkpoint 红线。
"""
...
def _infer_raw_bell_path(hdf5_path: Path, episode_num: str) -> Path:
    ...
    return Path(f"/mnt/data1/liujingzhi/dataset/click_bell/{setting}/data/episode{episode_num}.hdf5")
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        if raw_episode_path is None:
            ...
            if match:
                ...
                raw_episode_path = _infer_raw_bell_path(hdf5_path, episode_num)
...
def main():
    parser = argparse.ArgumentParser(description="Click Bell Analysis (3 phases)")
