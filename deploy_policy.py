...
def eval(TASK_ENV, model, observation):
    ...
    # Print subtask and optional debug tokens when available (PI05 with subtask decoding)
    if getattr(model, "_last_infer", None) is not None:
        last = model._last_infer
        if last.get("debug_tokens") is not None:
            print(f"[Debug Tokens]: {last['debug_tokens'].tolist()}")
        if last.get("subtask_text") is not None:
            print(f"[Pi0.5 Subtask]: {last['subtask_text']}")
        elif last.get("subtask_text") == "":
            print("[Pi0.5 Subtask]: (empty)")
