...
def create_torch_dataset(
    ...
    # Build delta_timestamps for actions
    delta_timestamps = {
        key: [t / dataset_meta.fps for t in range(action_horizon)] for key in data_config.action_sequence_keys
    }
    
    # Add delta_timestamps for observations if multi-frame stacking is enabled
    n_obs_steps = getattr(model_config, 'n_obs_steps', 1)
    if n_obs_steps > 1:
        # Get camera keys from dataset metadata
        camera_keys = dataset_meta.camera_keys
        # Add delta_timestamps for each camera (negative timestamps for past frames)
        for cam_key in camera_keys:
            delta_timestamps[cam_key] = [-(n_obs_steps - 1 - t) / dataset_meta.fps for t in range(n_obs_steps)]
        # Add delta_timestamps for state
...
def transform_dataset(
    ...
    *, 
    ...
    if model_config is not None:
        n_obs_steps = getattr(model_config, 'n_obs_steps', 1)
        if n_obs_steps > 1:
            # Extract image keys from repack transform structure
            image_keys = []
            for transform in data_config.repack_transforms.inputs:
                if isinstance(transform, _transforms.RepackTransform):
                    # RepackTransform structure: {"images": {"cam_high": "...", ...}, ...}
                    if "images" in transform.structure and isinstance(transform.structure["images"], dict):
                        image_keys = list(transform.structure["images"].keys())
                        break
...
def create_torch_data_loader(
    ...
    *,
    ...
    dataset = transform_dataset(dataset, data_config, model_config=model_config, skip_norm_stats=skip_norm_stats)
