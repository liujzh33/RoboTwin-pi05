#!/usr/bin/env python3
"""
grab_roller 轨迹分析：4 面板图（夹爪、末端高度 Z、速度、阶段划分）。
2 阶段：双臂靠近容器 → 双臂抓住容器。
"""
...
from task_definitions.grab_roller import GrabRollerProcessor
from task_definitions.trajectory_analyzer import TrajectoryAnalyzer

RAW_DATA_ROOT = "/mnt/data1/liujingzhi/dataset/grab_roller/aloha-agilex_randomized_500/data"
...
def analyze_episode(hdf5_path: Path, save_path: Path | None = None):
    ...
    task_processor = GrabRollerProcessor()
    ...
    axes[3].set_ylim(0, 1)
    axes[3].set_yticks([])
    axes[3].set_title(f"Predicted Phases (Total: {num_phases}) [Approach → Grasp]", fontweight="bold")
    ...
    for i in range(len(phases) - 1):
        ...
        short = "Approach" if i == 0 else "Grasp"
        axes[3].text(mid, 0.5, f"P{i}: {short}", ha="center", va="center", fontsize=10, fontweight="bold")

    # 阶段分界线（首次闭合处）
    if checkpoints:
        cp0 = int(checkpoints[0])
        for ax in axes:
            ax.axvline(x=cp0, color="red", linestyle="-", linewidth=2, alpha=0.9, zorder=5)
...
def main():
    parser = argparse.ArgumentParser(description="Grab Roller trajectory analysis (2 phases)")
