...
def main(
    ...
    # Use asset_id if available, otherwise use repo_id
    # If repo_id is an absolute path, use asset_id to save in assets directory
    asset_id = data_config.asset_id or data_config.repo_id
    # If asset_id is still an absolute path, extract a name from it
    if asset_id and (asset_id.startswith("/") or asset_id.startswith("gs://")):
        # For absolute paths, use the last component as asset_id
        asset_id = Path(asset_id).name or "default"
