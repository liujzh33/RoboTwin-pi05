#!/usr/bin/env python3
"""
dump_bin_bigbin 轨迹分析：4 面板图（夹爪、末端高度 Z、速度、阶段划分）。
风格与 analyze_trajectory_blocks_ranking_rgb.py 一致。
"""
...
from task_definitions.dump_bin_bigbin import DumpBinBigbinProcessor
from task_definitions.trajectory_analyzer import TrajectoryAnalyzer

# 原始数据路径，用于读取 endpose Z（若不存在则用 qpos 近似）
RAW_DATA_ROOT = "/mnt/data1/liujingzhi/dataset/dump_bin_bigbin/aloha-agilex_randomized_500/data"


def _get_raw_episode_path(hdf5_path: Path) -> Path | None:
    """从 processed 的 episode_*/episode_*.hdf5 推断 raw episode 路径"""
    match = re.search(r"episode_(\d+)", hdf5_path.name)
    if not match:
        return None
    ep_num = match.group(1)
    return Path(RAW_DATA_ROOT) / f"episode{ep_num}.hdf5"


def _short_phase_label(desc: str, i: int) -> str:
    """从子任务描述生成简短阶段标签（用于图例）"""
    d = desc.lower()
    if "approach" in d or "close" in d or "near" in d:
        return "Approach" if "right" not in d and "left" not in d else ("R-Approach" if "right" in d else "L-Approach")
    if "grasp" in d or "hold" in d or "gripper" in d:
        return "Grasp" if "right" not in d and "left" not in d else ("R-Grasp" if "right" in d else "L-Grasp")
    if "place" in d or "middle" in d or "center" in d:
        return "Place"
    if "tilt" in d or "pour" in d:
        return "Pour"
    return f"P{i}"
...
def analyze_episode(hdf5_path: Path, save_path: Path | None = None):
    ...
    task_processor = DumpBinBigbinProcessor()
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        # End-Effector Height (Z)：优先从 raw 数据读 endpose
        z_left = None
        z_right = None
    ...
    # Subplot 4: 阶段划分（色块 + 简短标签）
    axes[3].set_xlim(0, total_steps)
    axes[3].set_ylim(0, 1)
    axes[3].set_yticks([])
    legend_note = "[Approach → Grasp → Pour]" if num_phases == 3 else "[R-Approach, R-Grasp, Place, L-Approach, L-Grasp, Pour]"
    ...
    for i in range(len(phases) - 1):
        ...
        axes[3].text(mid, 0.5, f"P{i}: {short}", ha="center", va="center", fontsize=10, fontweight="bold")

    # 倾倒起始帧 = 最后一个 checkpoint（夹爪闭合后 Z 第一个峰），与图中红线一致
    pour_start = int(checkpoints[-1]) if checkpoints else None
    if pour_start is not None:
        for ax in axes:
            ax.axvline(x=pour_start, color="red", linestyle="-", linewidth=2, alpha=0.9, zorder=5)
...
def main():
    parser = argparse.ArgumentParser(description="Dump Bin Bigbin trajectory analysis (4 panels)")
