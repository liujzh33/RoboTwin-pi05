#!/usr/bin/env python3
"""
handover_mic 轨迹分析：4 面板图（夹爪、末端高度 Z、速度、阶段划分）。
3 阶段：靠近麦克风 → 抓住麦克风并移到中间 → 另一臂抓住并原臂放开。
"""
...
from task_definitions.handover_mic import HandoverMicProcessor
from task_definitions.trajectory_analyzer import TrajectoryAnalyzer

RAW_DATA_ROOT = "/mnt/data1/liujingzhi/dataset/handover_mic/aloha-agilex_randomized_500/data"
...
def analyze_episode(hdf5_path: Path, save_path: Path | None = None):
    ...
    task_processor = HandoverMicProcessor()
    ...
    axes[3].set_ylim(0, 1)
    axes[3].set_yticks([])
    axes[3].set_title(f"Predicted Phases (Total: {num_phases}) [Approach → Grasp+Move → Handover]", fontweight="bold")

    phases = [0] + list(checkpoints) + [total_steps]
    colors = ["#ccffcc", "#ccccff", "#ffffcc"]
    labels = ["Approach", "Grasp+Move", "Handover"]

    for i in range(len(phases) - 1):
        start, end = phases[i], phases[i + 1]
        mid = (start + end) / 2
        axes[3].axvspan(start, end, color=colors[i % len(colors)], alpha=0.5)
        axes[3].axvline(x=start, color="k", linestyle="-", linewidth=1)
        axes[3].text(mid, 0.5, f"P{i}: {labels[i]}", ha="center", va="center", fontsize=10, fontweight="bold")

    # 红线标出第三阶段起始（另一臂闭合）
    if len(checkpoints) >= 2:
        for ax in axes:
            ax.axvline(x=int(checkpoints[1]), color="red", linestyle="-", linewidth=1.5, alpha=0.8, zorder=5)
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description="Handover Mic trajectory analysis (3 phases)")
